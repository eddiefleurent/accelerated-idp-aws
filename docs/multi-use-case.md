...
**Supported Patterns:**
- Pattern 2: Full use-case-scoped configuration (classification, extraction, HITL, confidence thresholds)
- Pattern 1: Full use-case-scoped configuration (ProcessResults, summarization, evaluation, HITL)
...
```bash
# Read the use cases JSON file
USE_CASE_CONFIGS=$(cat config_library/pattern-2/multi-use-case-sample/use_cases.json)
...
A sample configuration preset is provided at `config_library/pattern-2/multi-use-case-sample/` with:
...
# Deploy with Pattern-2 using multi-use-case-sample config
aws cloudformation create-stack \
...
python3 -c "
import json
with open('config_library/pattern-2/multi-use-case-sample/use_cases.json') as f:
    data = json.load(f)
...
> **📝 Note:** Both patterns propagate `use_case_context` through the full workflow. The Queue Processor adds `use_case_context` to the Step Functions input, the state machine forwards it to every step, and each Lambda handler uses it to load use-case-scoped configuration via `get_config()`. When `use_case_context` is empty (flat S3 key uploads), all handlers fall back to the global default configuration.
