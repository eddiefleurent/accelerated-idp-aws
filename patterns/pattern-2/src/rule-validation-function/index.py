...
def handler(event, context):
    ...
    # Normalize missing or null use_case_context to an empty dict to avoid AttributeError
    ...
    # Log safe identifiers only (avoid exposing secrets from full config dump)
    logger.info(
        "Config loaded for business_unit_id=%s, use_case_id=%s, rule_validation_enabled=%s",
        effective_business_unit_id,
        effective_use_case_id,
        config.rule_validation.enabled,
    )
    ...
    # X-Ray annotations - use effective IDs (resolved from context + document)
